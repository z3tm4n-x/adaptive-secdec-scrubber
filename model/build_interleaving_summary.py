...
def add_smoke_section(lines: list[str], smoke_rows: list[dict[str, object]]) -> None:
    ...
    lines.append("")
    lines.append("| D | strategy | scrub cycles | corrected | DED detections | unique uncorrectable words | busy, % |")
    lines.append("|---:|---|---:|---:|---:|---:|---:|")
    ...
    lines.append(
        ...
        "При D=2 он раскладывается как 2+1 по двум словам, поэтому одна DED-группа остаётся. "
        "При D=3 он раскладывается как 1+1+1 по трём словам, и в данном smoke-прогоне "
...
def add_interval_sweep_section(lines: list[str], summary_rows: list[dict[str, object]]) -> None:
    ...
    lines.append("")
    lines.append("| D | fixed interval | runs | busy, % | corrected | DED detections | unique uncorrectable words |")
    lines.append("|---:|---:|---:|---:|---:|---:|---:|")
    ...
    lines.append(
        ...
        "из проверенных интервалов число уникальных неустранимых слов остаётся существенно выше нуля. "
        "При D=3 риск-метрика резко ниже и сильнее отражает накопительный механизм."
    )
    lines.append("")
...
def add_deltas_section(lines: list[str], deltas: list[dict[str, object]]) -> None:
    ...
    lines.append(
        ...
        "`unique_uncorrectable_words` относительно D=1 и D=2. "
        "Одновременно внутри D=3 увеличение интервала даёт статистически значимый рост unique, "
        "что указывает на возврат к накопительной, управляемой интервалом модели риска."
    )
    lines.append("")
...
def main() -> None:
    ...
    lines.append("")
    lines.append(
        "Техническое ограничение текущего Verilog-стенда — не более одной fault-инжекции "
        "за модельный такт. Поэтому для D>1 физически одномоментный кластер сериализуется "
        "по соседним тактам, а технический сдвиг фиксируется в `cluster_injection_skew`."
    )
    lines.append("")
