...
4. Актуальные interleaving-результаты используют истинную одновременную multi-slot инжекцию кластеров: группы одного физического кластера имеют одинаковый `time_cycle`, а `cluster_injection_skew = 0`. Устаревшие данные с сериализацией кластеров не следует использовать как основной вывод.
