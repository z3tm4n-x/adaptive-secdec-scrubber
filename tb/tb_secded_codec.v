...
module tb_secded_codec;

reg  [31:0] data_in;

wire [38:0] encoded_codeword;

reg  [38:0] decoder_input;
...
task check_encoded_word;
    begin
        if (encoded_codeword !== expected_codeword) begin
            $display("ERROR encoder mismatch at test %0d", test_count);
            $display("  data              = 0x%08h", data_in);
            $display("  expected_codeword = 0x%010h", expected_codeword);
            $display("  actual_codeword   = 0x%010h", encoded_codeword);
...
task check_no_error;
    begin
        decoder_input = encoded_codeword;
        #1;

        if (decoded_data !== data_in) begin
            $display("ERROR no-error data mismatch at test %0d", test_count);
            $display("  expected data = 0x%08h", data_in);
            $display("  actual data   = 0x%08h", decoded_data);
            error_count = error_count + 1;
        end

        if (corrected_codeword !== encoded_codeword) begin
            $display("ERROR no-error codeword mismatch at test %0d", test_count);
            $display("  expected corrected = 0x%010h", encoded_codeword);
            $display("  actual corrected   = 0x%010h", corrected_codeword);
            error_count = error_count + 1;
        end
...
task check_single_error;
    ...
    begin
        corrupted_codeword = encoded_codeword ^ (39'd1 << bit_index);
        decoder_input = corrupted_codeword;
        #1;
...
task check_double_error;
    ...
    begin
        ...
        if (single_error !== 1'b0 ||
            ...
            $display("ERROR double-error flags mismatch at test %0d bits %0d %0d",
                     test_count, bit_index_a, bit_index_b);
            $display("  single_error  = %0d", single_error);
            $display("  double_error  = %0d", double_error);
            $display("  uncorrectable = %0d", uncorrectable);
            error_count = error_count + 1;
        ...
        /*
         * Для двойной ошибки декодер не должен утверждать,
         * что восстановил исходное кодовое слово.
         * Поэтому здесь не проверяем corrected_codeword
         * на равенство encoded_codeword.
         */
    end
endtask

initial begin
    $dumpfile("results/logs/secded_codec.vcd");
    $dumpvars(0, tb_secded_codec);
    ...
    while (!$feof(file)) begin
        ...
        if (read_count == 2) begin
            ...
            /*
             * 1. Проверяем, что кодер совпадает с Python-эталоном.
             */
            check_encoded_word();

            /*
             * 2. Проверяем прохождение без ошибки.
             */
            check_no_error();

            /*
             * 3. Проверяем одиночную ошибку во всех 39 позициях.
             */
            ...
            /*
             * 4. Проверяем двойную ошибку во всех парах позиций.
             */
    ...
    if (error_count == 0) begin
        $display("SECDED codec test passed. Data words: %0d", test_count);
    end else begin
        $display("SECDED codec test failed. Errors: %0d", error_count);
        $fatal(1);
    end
